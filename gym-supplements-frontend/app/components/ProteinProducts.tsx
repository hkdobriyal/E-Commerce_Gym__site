...
// export default function SupplementsSection() {
//   const [supplementCategories, setSupplementCategories] = useState<Record<string, Product[]>>({});
...
//         setSupplementCategories(categories);
//       } catch (error) {
...
export default function SupplementsSection() {
  const [supplementCategories, setSupplementCategories] = useState<Record<string, Product[]>>({});
  const productsPerCategory = 5; // Number of products to display per category
  ...
  return (
    <section id="SupplementsSection" className="w-full px-4 py-8 bg-gray-100">
      ...
      {Object.entries(supplementCategories).map(([category, products]) => {
        const categorySlug = slugify(category);
        const limitedProducts = products.slice(0, productsPerCategory);
